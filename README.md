...
The subway station also contains a repeatable deep-expedition mode. Each floor is generated by Gemini from the current depth, player condition, carried loot, previous choice, and recent expedition history. While the player reads the current floor, the server pre-generates and caches the next one. Mechanical costs and rewards are resolved by the server within fixed bounds, and loot only enters the main inventory after a successful return.

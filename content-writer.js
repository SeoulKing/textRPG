...
function renderLocation(location) {
  ...
  const rooms = state.document.textRooms.filter(room => room.locationId === location.id);
  if (rooms.length) {
    $('.writer-single').insertAdjacentHTML('afterbegin', section('탐색 가능한 방', rooms.map(room => '<button type="button" class="related-link" data-open-room="' + esc(room.id) + '">' + esc(room.name) + ' · 엔티티 ' + room.entities.length + '개 →</button>').join('')));
    $$('[data-open-room]').forEach(button => button.onclick = () => go('textRooms', button.dataset.openRoom));
  }

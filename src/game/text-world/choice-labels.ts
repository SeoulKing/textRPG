...
export function resolveChoiceLabels(context: NarrativeContext, raw: unknown): ChoiceLabel[] {
  ...
  return (context.nextChoices ?? []).map(option => {
    ...
    const verb = option.family === "SOCIAL" ? /말|대화|이야기/ : option.family === "DISTRACT" ? /던진|던져/ : option.family === "ATTACK" ? /공격|친다|휘두|내리|찌르|겨눈/ : option.family === "NEGOTIATE" ? /말|설득|대화|진정/ : option.id.startsWith("journey:") ? /내려|올라|돌아|진입|들어|향/ : option.family === "TRADE" ? /산다|사 둔다|구입|구매|바꾼|교환/ : option.id.startsWith("delivery:") ? /건넨|건네|맡긴|전달/ : /^(work|care):/.test(option.id) ? /돕|도울|보탠|거든|거들|정찰|처치|돌본|치료|작업|수리|보수/ : option.family === "HANDLE" ? option.id.startsWith("stow:") ? /챙겨|보관/ : /꺼내|고쳐|쥐|쥔|든다/ : /^(harvest|toolwork):/.test(option.id) ? /벌목|모으|모은|구하|구한|수색|뒤지|뒤진|꼬|잘라|낚|손질|작업/ : option.id.startsWith("defocus") ? /시선|관심/ : option.family === "PLACE_OBJECT" ? /넣|놓|내려|담/ : option.family === "MOVE_OBJECT" ? /밀|옮/ : option.family === "COLLECT" ? /챙|집|쥐|거두|수거|꺼내/ : option.family === "TOOL" ? /켜|켠|끄|끈|비추|비춘|집어/ : option.family === "OPEN_CONTAINER" || option.family === "ACCESS" ? /열|닫|살피|살펴|확인|잠금/ : /살피|살펴|시선|관심|다가|이동|돌아|향|나온|나서|기다|숨|낮춰|빠져|일어/;
    const validLabel = label && !/[\n\r.!?]|당신|플레이어|주인공|발견했다|획득했다|성공|실패|(?:result|entity|contents):/.test(label) && (option.id.startsWith("repair:") ? /수리|손본|보수/.test(label) : option.id.startsWith("tool:") ? /열|벌려|자르|잘라|부수|충격/.test(label) : verb.test(label))
      ...
      && (!/^(hide|emerge):/.test(option.id) || (option.label.includes(" 아래") ? /아래|밑/.test(label) && !/ 뒤/.test(label) : /뒤/.test(label) && !/아래|밑/.test(label)))
      ...
      && (option.family !== "DISTRACT" || /한 개/.test(label) && label.includes(option.label.split(" 한 개를 ")[1]?.replace(/으로 던진다$/, "") ?? option.label))

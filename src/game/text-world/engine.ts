...
import { passageBlockers, relocateEntity } from "./spatial";
...
const seconds: Record<WorldAction["type"], number> = { LOOK: 3, INSPECT: 5, MOVE: 5, POSTURE: 1, UNLOCK: 3, OPEN: 2, CLOSE: 2, TAKE: 3, LIGHT: 1, SURVEY: 5, LEAVE: 5, PUSH: 8, PUT: 3, DROP: 2, WAIT: 5, HIDE: 2, DEFOCUS: 2, FOCUS: 1 };
export function validateWorldAction(world: TextWorld, state: GameState, action: WorldAction): string | null {
  ...
  switch (action.type) {
    ...
    case "LIGHT": return !c.light ? "불을 켤 수 있는 물건이 아니다." : !c.light.on && c.light.fuelSeconds === 0 ? "조명의 에너지가 다해 켤 수 없다." : null;
  ...
  for (const action of actions) {
    ...
    switch (action.type) {
      ...
      case "TAKE": {
        ...
        world.player.heldItemId = e!.id; world.player.manipulating = true;
        relocateEntity(world, e!, { zone: "player" });
        if (item.itemId) state.inventory[item.itemId] = (state.inventory[item.itemId] ?? 0) + item.amount;
        if (c!.light) world.player.heldToolId = e!.id;
        ...
        after = { zone: c!.position.zone, name: e!.name, amount: item.amount, itemId: item.itemId, held: Boolean(c!.light) }; break;
      }
    ...
    const elapsed = options.advanceTime === false ? 0 : action.type === "WAIT" ? action.durationSeconds ?? 5 : seconds[action.type];
    advanceWorldSimulation(world, elapsed, event.id); elapsedSeconds += elapsed;

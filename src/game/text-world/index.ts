...
import type { ActionChoice, GameAction, GameState, SceneCard } from "../schemas";
...
export function textWorldEntryActions(state: GameState): ActionChoice[] {
  ...
  return [{ id: "text-world:enter", label: "옆쪽 역무실을 둘러본다", outcomeHint: "탐색과 물건 수집", showOutcomeHint: true,
    isAvailable: true, action: { type: "text_world", command: "enter" } }];
}

export function textWorldActions(state: GameState): ActionChoice[] {
...
export function textWorldScene(state: GameState): SceneCard | null {
  const world = state.textWorld;
  if (!world?.active) return null;
  return { id: "text-world:" + world.sceneRevision, locationId: "subway", title: worldRooms(world)[world.player.zone].name,
    paragraphs: world.lastParagraphs, choices: textWorldActions(state).map(({ action, ...choice }) => ({ ...choice, serverActionHint: action })),
    materialIds: { locationIds: ["subway"], personIds: [], itemIds: [] }, source: world.source, generatedAt: new Date(0).toISOString() };
}

export async function performTextWorldAction(
  state: GameState, action: Extract<GameAction, { type: "text_world" }>, gameId: string,
  narrator: TextWorldNarrator = narrateTextWorld, rooms?: TextRoom[],
) {

...
export function focusOptions(world: TextWorld): WorldOption[] {
  ...
  for (const entity of visibleEntities(world)) {
    if (entity.id === context.focusEntityId || entity.components.position.zone === "player" || world.entities[entity.components.position.zone]) continue;
    const c = entity.components, known = world.observations[entity.id];
    if ((known?.inspected || known?.stages.includes("interior")) && !c.container?.items.length && !c.openable && !c.light && !c.physical?.movable && c.physical?.supportCapacity === undefined && !c.portal) continue;

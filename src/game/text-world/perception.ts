...
export function perceiveWorld(world: TextWorld): WorldFact[] {
  ...
  for (const e of visibleEntities(world)) {
    ...
    if (c.structure?.integrity !== 0 && lit && (near || carriedByPlayer(world, e)) && (!c.openable || c.openable.isOpen || c.physical?.opaque === false) && c.container && (!c.expeditionCache || world.observations[e.id]?.inspected)) {

...
export function buildNarrationPrompt(context: NarrativeContext) {
  ...
  const instructions = [EXPLORATION_PROSE_STYLE,
    "한국어 탐색 소설을 렌더링한다. intent는 시도, results는 실제 실행 순서다. requiredFacts는 행동 결과와 현재 관찰을 모두 포함하는 필수 전달 목록이다. 성공·실패·발견·변화뿐 아니라 이 목록에 남은 공간 정보도 빠짐없이 전달하고 선택하지 않은 행동을 완료하지 않는다. direction.beats는 원인에 감각과 발견을 연결하는 안내이며 문단별 역할은 고정하지 않는다.",
    ...
    "paragraphCount를 지킨다. JSON의 첫 속성 paragraphs를 먼저 완성하고 그 뒤 choiceLabels를 쓴다. {paragraphs:[{factIds,text}],choiceLabels:[{optionId,label,thought}]}만 반환한다. 각 문단은 factIds를 먼저 고른 뒤 그 사실들을 함께 담는 text를 쓴다. 앞 문단에서 다루지 않은 필수 사실은 다음 문단에 연결한다. factIds에는 해당 문단에서 실제 표현한 사실 ID만 쓴다. 본문에 ID·선택지·구조 설명을 노출하지 않는다.",
  ...
  if(context.results.some(e=>["ACTOR_ACTIVITY","NPC_CONSUME"].includes(e.type))) instructions.push("ACTOR_ACTIVITY와 NPC_CONSUME는 NPC가 실제로 한 행동이다. 플레이어 행동으로 바꾸지 않으며, 먹은 물건·양과 이동한 공간은 결과에 근거한다. NPC의 의도·배고픔 수치나 보지 못한 행동을 추측하지 않는다.");
  ...
  instructions.push("최종 확인: 이번 필수 사실 ID는 " + JSON.stringify(context.requiredFacts.map(f => f.id))
    + "이다. 각 사실의 내용을 본문에서 실제로 표현하고 해당 문단의 factIds에 연결한다. paragraphs 전체 factIds의 합집합에 이 ID가 모두 있어야 한다. ID만 붙여 누락된 서술을 대신하지 않는다. optionalFacts는 모두 쓸 필요가 없다.");

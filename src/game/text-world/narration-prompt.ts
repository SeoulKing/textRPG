...
export function buildNarrationPrompt(context: NarrativeContext) {
  ...
  const instructions = [EXPLORATION_PROSE_STYLE,
    ...
    "response 감각은 action에 적힌 실제 조작 때만 발생한 반응이다. 한 장면에서 감각 한두 개를 움직임 속에 섞고, 그 뒤 새로 드러난 정보에 호흡을 준다. 첫 문단은 1~2문장으로 읽을 거리를 빠르게 시작하고 남은 발견은 이어지는 문단에 쓴다. 손에 든 물건이나 낮춘 자세를 끝에 상태표처럼 덧붙이지 말고 지금 동작에 연결한다.",

...
export function textRoomIssues(rooms: TextRoom[], itemIds: Set<string>) {
  ...
  for (const room of rooms) {
    ...
    for (const entity of room.entities) {
      ...
      if (c.position.zone !== room.id && !(parent?.components.container && parent.components.position.zone === room.id && parent.components.container.items.includes(entity.id))) add(room.id, `${label}: 배치할 방이나 보관함을 확인해 주세요.`);
      ...
      if (c.container) {
        if (!c.openable || c.portable || c.portal || c.light) add(room.id, `${label}: 보관함은 고정된 여닫는 물체여야 합니다.`);
        ...
        for (const id of c.container.items) {
          ...
          if (!child?.components.portable || child.components.position.zone !== entity.id || child.components.container || child.components.portal || child.components.light) add(room.id, `${label}: 내용물 '${id}'는 이 보관함에 배치된 수집 아이템이어야 합니다.`);
        }
      }
      if (c.portable?.itemId && !itemIds.has(c.portable.itemId)) add(room.id, `${label}: 지급할 아이템을 찾을 수 없습니다.`);
      if (c.light && (parent || c.portal || (c.portable && c.portable.itemId !== null))) add(room.id, `${label}: 조명은 방에 배치하고, 휴대 조명은 손에 드는 물건으로 설정해 주세요.`);

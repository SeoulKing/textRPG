...
export const arcanaPlazaChoices: ActionDefinition[] = [
  interactionFor("arcana_plaza", {
    ...
    label: "달빛 약초를 채집한다",
    type: "search",
    outcomeHint: "달빛 약초 +1 / +20분",
    showOutcomeHint: true,
    effects: [
      { type: "add_item", itemId: "moonHerb", amount: 1 },
      { type: "advance_time", minutes: 20 },
      { type: "log", message: "당신은 분수 가장자리에서 은빛으로 빛나는 달빛 약초 한 줌을 채집했다." },
      { type: "set_random_scene", tag: "arcana:result:moon-herb" },
    ],
    tags: ["magic-resource", "alchemy", "repeatable"],
    riskHint: "low",
  }),
  interactionFor("arcana_plaza", {
    ...
    label: "부유 룬을 해독한다",
    type: "explore",
    outcomeHint: "MP -1 / 비전 가루 +1 / +15분",
    showOutcomeHint: true,
    presentationMode: "always",
    conditions: [{ type: "stat_gte", stat: "mind", value: 1 }],
    failureNote: "룬을 해독할 MP가 부족하다.",
    failureEffects: [
      { type: "log", message: "빛의 문장을 붙잡으려 했지만, 남은 마력으로는 룬의 흐름을 따라갈 수 없다." },
    ],
    effects: [
      { type: "change_stat", stat: "mind", value: -1 },
      { type: "add_item", itemId: "arcaneDust", amount: 1 },
      { type: "advance_time", minutes: 15 },
      { type: "log", message: "당신은 허공의 룬을 손끝으로 따라 그려 비전 가루 한 줌을 응축했다." },
      { type: "set_random_scene", tag: "arcana:result:arcane-dust" },
    ],
    tags: ["magic-resource", "rune", "repeatable", "mp-cost"],
    riskHint: "medium",
  }),
  interactionFor("arcana_plaza", {

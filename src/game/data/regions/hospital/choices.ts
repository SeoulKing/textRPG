...
export const hospitalChoiceDefinitions: ChoiceDefinition[] = [
  sceneChoice({
    ...
    label: "{{item:radioBattery|을를}} 챙긴다",
    outcomeHint: "묵직한 배터리를 가방 안쪽에 고정한다. 구조 신호 장비의 첫 핵심 부품이다.",
    ...collectStockItemChoiceParts({
      ...medicineCabinet,
      itemId: "radioBattery",
      logMessage: "당신은 보관함 깊숙한 곳에서 {{item:radioBattery|을를}} 꺼내 가방 안쪽에 고정한다.",
      minutes: 15,
    }),
    riskHint: "medium",
  }),
  sceneChoice({
    ...
    outcomeHint: "남은 {{item:painRelief|을를}} 챙겨, 부상에 대비한다.",
    ...collectStockItemChoiceParts({
      ...medicineCabinet,
      itemId: "painRelief",
  ...
  sceneChoice({
    ...
    label: "붕대로 쓸 천을 챙긴다",
    outcomeHint: "깨끗한 {{item:clothScrap|을를}} 챙겨 제작과 응급처치에 대비한다.",
    ...collectStockItemChoiceParts({
      ...medicineCabinet,
      itemId: "clothScrap",
      logMessage: "당신은 아직 깨끗한 {{item:clothScrap|을를}} 접어 넣는다.",
      minutes: 15,
    }),
    riskHint: "low",
  }),
  sceneChoice({
    ...
    label: "수액줄을 챙긴다",
    outcomeHint: "멀쩡한 수액줄과 붕대끈을 챙겨, 도구와 장비를 고정할 {{item:cordage|으로로}} 쓴다.",
    ...collectStockItemChoiceParts({
      ...medicineCabinet,
      itemId: "cordage",

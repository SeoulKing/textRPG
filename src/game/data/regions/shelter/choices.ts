...
export const shelterChoiceDefinitions: ChoiceDefinition[] = [
  ...
  sceneChoice({
    id: "craft_wood_plank",
    label: "{{item:woodPlank}} 가공",
    outcomeHint: "{{item:wood}} 1개를 가공해 {{item:woodPlank}} 1개를 만든다. 가공 시간 10분.",
    showOutcomeHint: true,
    presentationMode: "always",
    conditions: [
      { type: "has_item", itemId: "wood", amount: 1 },
    ],
    failureNote: "가공하려면 {{item:wood}} 1개가 필요하다.",
    failureEffects: [
      { type: "log", message: "가공할 {{item:wood|이가}} 없다. 숲에서 목재를 구해 와야 한다." },
    ],
    effects: [
      { type: "remove_item", itemId: "wood", amount: 1 },
      { type: "add_item", itemId: "woodPlank", amount: 1 },
      { type: "log", message: "당신은 {{item:wood}}의 거친 면을 다듬어 단단한 {{item:woodPlank}} 한 장을 만든다." },
  ...
  sceneChoice({
    ...
    effects: [
      { type: "remove_item", itemId: "wood", amount: 1 },
      { type: "add_item", itemId: "firewood", amount: 4 },
      { type: "log", message: "당신은 {{item:wood|을를}} 잘게 쪼개 불을 피우기 좋은 {{item:firewood}} 네 개를 마련한다." },
      { type: "advance_time", minutes: 10 },
    ],
    nextSceneId: "shelter_crafting_menu",
    riskHint: "low",
  ...
  sceneChoice({
    ...
    conditions: [
      ...
      { type: "has_item", itemId: "firewood", amount: 1 },
    ],
    failureNote: "간이 화로와 {{item:dentedPot|이가}} 있어야 하고, {{item:rawRice}} 1 / {{item:vegetables}} 1 / {{item:waterBottle}} 1 / {{item:firewood}} 1이 필요하다. 효과: +1 정신력 / +6 기력.",
    failureEffects: [
      { type: "log", message: "{{item:hotMeal|을를}} 만들려면 화로와 {{item:dentedPot}}, {{item:rawRice|과와}} {{item:vegetables}}, {{item:waterBottle}}, 불씨로 쓸 {{item:firewood}} 한 개가 필요하다." },
    ],
    effects: [
      ...
      { type: "remove_item", itemId: "firewood", amount: 1 },
  ...
  sceneChoice({
    ...
    failureNote: "{{item:ricePorridge|은는}} 간이 화로와 {{item:dentedPot|이가}} 있어야 하고, {{item:rawRice}} 1 / {{item:waterBottle}} 1 / {{item:firewood}} 1이 필요하다.",
    failureEffects: [
      { type: "log", message: "{{item:ricePorridge|을를}} 끓이려면 {{item:dentedPot}}, {{item:rawRice}}, {{item:waterBottle}}, 불씨로 쓸 {{item:firewood|이가}} 필요하다." },
    ],
    effects: [
      { type: "remove_item", itemId: "rawRice", amount: 1 },
      { type: "remove_item", itemId: "waterBottle", amount: 1 },
  ...
  sceneChoice({
    ...
    failureNote: "{{item:grilledFish|은는}} 간이 화로가 있어야 하고, {{item:riverFish}} 1 / {{item:firewood}} 1이 필요하다.",
    failureEffects: [
      { type: "log", message: "{{item:grilledFish|을를}} 만들려면 간이 화로와 {{item:riverFish}} 한 마리, 불씨를 살릴 {{item:firewood}} 한 개가 필요하다." },
    ],
    effects: [

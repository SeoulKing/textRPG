...
export const subwayChoiceDefinitions: ChoiceDefinition[] = [
  sceneChoice({
    ...
    label: "{{item:radioAntenna|을를}} 떼어 낸다",
    ...
    ...collectStockItemChoiceParts({
      ...
      logMessage: "당신은 녹슨 나사를 비틀어 {{item:radioAntenna|을를}} 떼어 낸다. 어둠 속 금속음이 한동안 귀에 남는다.",
      minutes: 15,
    }),
    riskHint: "high",
  }),
  sceneChoice({
    ...
    label: "신호함 {{item:scrapMetal|을를}} 챙긴다",
  ...
  sceneChoice({
    ...
    label: "미개봉 {{item:waterBottle|을를}} 챙긴다",
    outcomeHint: "역무실 책상 아래 굴러 들어간 {{item:waterBottle|을를}} 꺼낸다.",
    ...collectStockItemChoiceParts({
      ...signalBox,
      itemId: "waterBottle",
      logMessage: "당신은 책상 아래 굴러 들어간 미개봉 {{item:waterBottle|을를}} 꺼낸다.",
      minutes: 15,
    }),
    riskHint: "low",
  }),

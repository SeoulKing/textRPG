...
import {
  ...
  inactiveStockNodeCondition,
  leaveStockNodeChoiceParts,
} from "../../stock-node-choice-helpers";
...
export const convenienceChoiceDefinitions: ChoiceDefinition[] = [
  ...
  sceneChoice({
    ...
    conditions: [inactiveStockNodeCondition("convenience_register")],
    effects: [
  ...
  sceneChoice({
    ...
    ...collectStockItemChoiceParts({
      ...supplyPile,
      itemId: "woodPlank",
      logMessage: "당신은 아직 단단한 판자들을 한데 모아 어깨에 걸친다.",
      minutes: 10,
    }),
    riskHint: "low",
  }),
  ...
  sceneChoice({
    ...
    ...collectStockItemChoiceParts({
      ...supplyPile,
      itemId: "scrapMetal",
      logMessage: "당신은 선반 모서리에 걸린 금속 부품을 비틀어 떼어 낸다.",
      minutes: 5,
    }),
    riskHint: "low",
  }),
  sceneChoice({
    ...
    ...leaveStockNodeChoiceParts(shelf.nodeId, "당신은 손에 쥔 것을 확인한 뒤 조심스럽게 몇 걸음 물러선다."),
    riskHint: "low",
  }),
  sceneChoice({
    ...
    ...leaveStockNodeChoiceParts(supplyPile.nodeId, "당신은 자재 더미에서 챙길 것을 추린 뒤 다시 가게 안쪽으로 몸을 뺀다."),
    riskHint: "low",
  }),
  sceneChoice({

...
async function game(){
 ...
 async function choose(id){if(id.startsWith('explore:')&&!offered(id)&&offered('defocus'))await choose('defocus');const selected=offered(id),action=selected?.action??(id.startsWith('travel:')?{type:'travel',targetId:id.slice(7)}:id.startsWith('use:')?{type:'use_item',itemId:id.slice(4)}:undefined);
...
const radioRoute=['opening_commit','accept_first_canned_food_quest','travel:convenience','travel:hospital','explore:hospital_cabinet','collect:hospital_cabinet','travel:convenience','travel:shelter','travel:subway','travel:office','explore:subway_signal_box','collect:subway_signal_box','leave','travel:checkpoint','explore:checkpoint_radio_truck','collect:checkpoint_radio_truck','travel:subway','travel:shelter','open_shelter_crafting','assemble_rescue_radio','leave_shelter_crafting'];

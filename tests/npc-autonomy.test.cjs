...
test('the actual service keeps JSONB choices stable and a gifted meal becomes later NPC consumption with no extra generation',async()=>{
 ...
 await choose(r=>r.action.type==='npc_dialogue'&&r.action.command==='start');assert.equal(dialogues,1);assert.match(lastInput.context.actor.placement,/역무실/);assert(!lastInput.context.location.sceneParagraphs.join(' ').includes('기둥'));
 ...
 const world=saved.state.textWorld;assert.equal(saved.state.npcDialogue.conversations.shumi.inventory.cannedFood,0);assert.equal(saved.state.inventory.cannedFood,1);assert(world.events.some(e=>e.type==='NPC_CONSUME'));assert.equal(dialogues,2);assert.equal(narrations,1);assert.equal(world.entities.shumi_presence.components.actor.life.mode,'REST');const prose=snapshot.currentScene.paragraphs.join(' ');assert.match(prose,/슈미가 캔 음식 1개를 먹는다/);assert.equal((prose.match(/자리에 머물며 쉬고 있다/g)||[]).length,1);assert(!prose.includes('주변을 살피고 있다'));

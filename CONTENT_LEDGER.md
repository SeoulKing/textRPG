...
| 구현됨 | `hotMeal` | 따뜻한 식사 | food | uncommon | 4,500원 | 정신력 +1 / 기력 +4 / 탈진 완화 +3 | 0분 | 급식소 구매·배식권 교환·일 돕기, 거처 요리. 사용 시 오늘의 식사 확보로 인정된다. |
...
| 구현됨 | `vegetables` | 채소 | food | common | 600원 | 효과 없음 | 0분 | 급식소 식재료 상자. 따뜻한 식사와 숲죽 재료다. |
| 구현됨 | `wildGreens` | 산나물 | food | common | 400원 | 기력 +1 | 10분 | 숲 채집. 그대로 먹거나 나물국·숲죽 재료로 사용한다. |
...
| 구현됨 | `greensSoup` | 나물국 | food | common | 0원 | 정신력 +1 / 기력 +3 / 탈진 완화 +2 | 10분 | 거처 요리. 숲 채집물을 소모하는 가벼운 회복식이다. |
| 구현됨 | `forestStew` | 숲죽 | food | uncommon | 0원 | 기력 +6 / 탈진 완화 +2 | 10분 | 거처 요리. 산나물과 채소를 많이 쓰는 고효율 기력식이다. |
...
| 구현됨 | `rationTicket` | 배식권 | ticket | common | 0원 | 직접 사용 불가 | 검문소, 급식소 일 돕기 | 급식소에서 따뜻한 식사 1개로 교환 |
| 구현됨 | `woodPlank` | 목재 판자 | material | common | 700원 | 직접 사용 불가 | 편의점, 숲 | 거처 보강, 화로, 손도끼, 요리 연료 |
...
| 구현됨 | `manaShard` | 마력 결정 파편 | material | rare | 0원 | 직접 사용 불가 | 마법도시 포탈 최초 통과 | 룬 나침반 제작 |
...
모든 요리는 간이 화로와 찌그러진 냄비가 필요하며, 냄비 내구도 1을 소모한다.

| 상태 | 결과 | 소모 재료 | 조리 시간 | 완성품 사용 효과 |
|---|---|---|---:|---|
| 구현됨 | 따뜻한 식사 1 | 쌀 1 / 채소 1 / 물병 1 / 목재 판자 1 | 0분 | 정신력 +1 / 기력 +4 / 탈진 완화 +3 |
| 구현됨 | 묽은 죽 1 | 쌀 1 / 물병 1 / 목재 판자 1 | 20분 | 정신력 +1 / 기력 +4 / 탈진 완화 +2 |
| 구현됨 | 나물국 1 | 산나물 1 / 물병 1 / 목재 판자 1 | 15분 | 정신력 +1 / 기력 +3 / 탈진 완화 +2 |
| 구현됨 | 숲죽 1 | 산나물 2 / 채소 1 / 물병 1 / 목재 판자 1 | 25분 | 기력 +6 / 탈진 완화 +2 |
...
| 검토 필요 | 조리 음식의 식사 인정 | 비상식량·캔 음식·따뜻한 식사만 `mealSecured`를 켠다. 묽은 죽·나물국·숲죽은 켜지 않는다. | 모든 완성 요리를 식사로 인정할지 결정 |
| 검토 필요 | 즉시 사용 아이템 | 비상식량·따뜻한 식사·물병·진통제는 사용 시간이 0분이다. | 공통 사용 시간을 둘지 결정 |
| 검토 필요 | 따뜻한 식사 조리 시간 | 다른 요리와 달리 제작 시 시간이 흐르지 않는다. | 조리 시간을 추가할지 결정 |
...
| 구현됨 | `forest` | 숲 | low | 시간과 도구 내구도를 자원으로 바꾸는 반복 채집 지역 | 목재, 끈, 산나물, 눅눅한 빵, 캔 음식, 고철, 천 | 임시 거처, 편의점 폐허, 급식소 |
| 구현됨 | `hospital` | 작은 병원 | medium | 돈을 내고 치료하거나 일을 도우며 약품·배터리를 확보하는 장소 | 진통제, 천, 끈, 무전기 배터리 | 편의점 폐허 |
| 구현됨 | `subway` | 지하철역 | high | 안테나 확보와 장기 심층 탐험을 시작하는 고위험 지역 | 무전기 안테나, 고철, 끈, 천, 물, 식량, 진통제 | 급식소, 검문소 |
| 구현됨 | `checkpoint` | 검문소 | high | 송신기와 구조 소문을 확인하고 위험한 외곽 정찰을 하는 후반 지역 | 무전기 송신기, 진통제, 배식권, 고철, 끈 | 지하철역 |
| 구현됨 | `magic_city_entrance` | 마법도시 입구 | medium | 편의점 옥상에서 현실과 확장 세계를 잇는 포탈을 발견하고 통과하는 경계 지역 | 마력 결정 파편 | 편의점 폐허, 포탈 너머 아르카나 광장 |
...
|---|---|---|
| 숲 벌목 | 30분 | 목재 판자 3 |
| 숲 손도끼 벌목 | 30분 / 손도끼 내구도 1 | 목재 판자 5 |
| 숲 일반 수색 | 30분 | 빈손 50% / 캔 음식 1 10% / 목재 1 20% / 고철 1 10% / 천 1 10% |
| 숲 끈 만들기 | 30분 | 끈 묶음 2 |
| 숲 간이 칼 덩굴 채집 | 30분 / 간이 칼 내구도 1 | 끈 묶음 4 |
| 숲 먹을거리 채집 | 30분 | 빈손 55% / 산나물 1 25% / 눅눅한 빵 1 10% / 천 1 10% |
| 숲 간이 칼 먹을거리 수색 | 30분 / 간이 칼 내구도 1 | 빈손 35% / 산나물 1 35% / 눅눅한 빵 1 15% / 캔 음식 1 10% / 천 1 5% |
| 급식소 일 돕기 | 기력 1 / 40분 | 1,200원 40% / 배식권 1 25% / 따뜻한 식사 1 20% / 물병 1 15% |

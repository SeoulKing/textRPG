...
if(typeof module!=='undefined')module.exports={studioScenePool,studioWriteScenePool,studioChoiceResultSummary};
...
function renderScenePool(owner,root,story,onChange) {
  ...
  const draw=()=>{
    ...
    if(selected){
      ...
      $$('[data-pool-text]',root).forEach(input=>input.oninput=()=>{const i=Number(input.dataset.poolText);if(scene.blocks)scene.blocks[i].text=input.value;else scene.paragraphs[i]=input.value;const preview=$(`[data-pool-scene="${scene.id}"] small`,root);preview.textContent=resolveItemTextPreview((scene.blocks?.map(block=>block.text)??scene.paragraphs).join(' ')).slice(0,65);onChange();});

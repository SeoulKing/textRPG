...
function paintLivePreview() {
  ...
  $('[data-live-body]',root).innerHTML=`<div class="live-position"><strong>${esc(context.location?.name ?? '지역 미정')} · ${frame?.day ?? live.setup?.day ?? 1}일째</strong><span>${esc(context.scene?.title || '새 장면')}</span>${frame?.stats?`<small>체력 ${frame.stats.hp} · 기력 ${frame.stats.energy} · 정신력 ${frame.stats.mind}</small>${(frame.conditionCards??[]).map(condition=>`<small class="live-condition-status">${esc(condition.label)} Lv${condition.level} · 다음 피해 ${condition.nextDamageMinutes}분${condition.nextWorseningMinutes===null?'':` · 악화 ${condition.nextWorseningMinutes}분`}${condition.level===3?' · 추가 누적 시 생존 종료':''}</small>`).join('')}`:''}</div>

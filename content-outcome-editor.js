...
function renderResultBody(owner,root,story,{onChange,connection,rewards}) {
  ...
  StudioItemTextEditor.mount($('[data-result-text]',root),logs.map(effect=>effect.message).join('\n\n'),{key:`${state.selectedId}:${owner.id??state.selectedChoiceId}:result:${root.closest('[data-random]')?.dataset.random??'base'}:${root.closest('[data-detail]')?.dataset.detail??''}`});
